...
Mykeywordmatrix<-function(content,keyword){
  ...
  colnames(result)<-c(99:110)
  return(result[,2:12])
}

data.doc<-c(data100,data101,data102,data103
...
#issue<-Mykeywordmatrix(content,c("","","","","","","","","","","","","","","","","","",""))
...
dir<-paste("./excel檔/政府出版品/",1:28,".csv",sep="")
for (i in 1:28){
...
plotfunc<-function(Finallmatrix){
  ...
  pro.x<-ggplot(Finallmatrix,aes(
    ...
    scale_x_continuous(limits = c(1,11),breaks=c(1:11),labels = c("100","101","102","103","104","105","106","107","108","109","110"))+
  ...
  pro.age<-pro.x+pro.x2+pro.x3+pro.x4+pro.x5+pro.x6+pro.x7+pro.x8+pro.x9+pro.x10+pro.x11
  return(pro.age)
}

dir<-paste("./圖片檔/政府出版品/",1:28,".png",sep="")
for (i in 1:28){
...
colnames(issuematrix) <- c(100:110)
write.csv(issuematrix,file="C:/Users/JOEZ/OneDrive/桌面/R/國發會/excel檔/issue.csv",row.names = TRUE,fileEncoding = "Big5")
...
row.names(fieldmatrix) <- c("人口結構與生養需求","經濟就業與居住資源",
                            "數位轉型與科技影響","氣候變遷與環境挑戰")
colnames(fieldmatrix) <- c(100:110)
write.csv(fieldmatrix,file="C:/Users/JOEZ/OneDrive/桌面/R/國發會/excel檔/field.csv",row.names = TRUE,fileEncoding = "Big5")

issuematrix.plot <- plotfunc(issuematrix)
fieldmatrix.plot <- plotfunc(fieldmatrix)
png("./圖片檔/政府出版品/issue.png",width = 850,height = 600,res = 90,bg = "transparent")
print(issuematrix.plot)
dev.off()

png("./圖片檔/政府出版品/field.png",width = 850,height = 600,res = 90,bg = "transparent")
