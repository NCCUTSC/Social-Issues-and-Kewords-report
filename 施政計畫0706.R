...
Mykeywordmatrix<-function(content,keyword){
  ...
  colnames(result)<-c(103:110)
  return(result[,2:8])
}

data.doc<-c(data104,data105,data106,data107
...
dir<-paste("./excel檔/施政計畫/",1:28,".csv",sep="")
for (i in 1:28){
...
dir<-paste("./圖片檔/施政計畫/",1:28,".png",sep="")
for (i in 1:28){
...
colnames(issuematrix) <- c(104:110)
write.csv(issuematrix,file="C:/Users/JOEZ/OneDrive/桌面/R/國發會/excel檔/施政計畫/issue.csv",row.names = TRUE,fileEncoding = "Big5")
...
row.names(fieldmatrix) <- c("人口結構與生養需求","經濟就業與居住資源",
                            "數位轉型與科技影響","氣候變遷與環境挑戰")
colnames(fieldmatrix) <- c(104:110)
write.csv(fieldmatrix,file="C:/Users/JOEZ/OneDrive/桌面/R/國發會/excel檔/施政計畫/field.csv",row.names = TRUE,fileEncoding = "Big5")

issuematrix.plot <- plotfunc(issuematrix)
fieldmatrix.plot <- plotfunc(fieldmatrix)
png("./圖片檔/施政計畫/issue.png",width = 850,height = 600,res = 90,bg = "transparent")
print(issuematrix.plot)
dev.off()

png("./圖片檔/施政計畫/field.png",width = 850,height = 600,res = 90,bg = "transparent")
